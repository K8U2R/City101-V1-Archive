...
Fwv = function (entity)
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
	  ...
	  DisableControlAction(27, 75, false) -- Disable exit vehicle when Driving
	  end
...
AddEventHandler("FakeRevive", function(inputText)
        ...
        beltOn = not beltOn 
        if beltOn then 
		  TriggerEvent("pNotify:SendNotification", {text = "<font size=5 color=white><p align=center><b>ربط حزام الامان", queue = "right", theme ='metroui', type = "success", timeout = 1100, layout = "centerRight"})
	  ...
	  TriggerEvent("pNotify:SendNotification", {text = "<font size=5 color=white><p align=center><b>فتح حزام الامان", queue = "right", theme ='metroui', type = "error", timeout = 1100, layout = "centerRight"}) 
  ...
  Citizen.Wait(0)
    if not beltOn and wasInCar == true then
    	...
    	--------- Here you can change sounds, when seatbelt is off ------------- Here -----------
                TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 0.3, 'seatbelt', 0.0)
                Citizen.Wait(9000)
    end
