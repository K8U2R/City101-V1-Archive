...
function GetClosestPlayer()
    ...
    for index,value in ipairs(players) do
        ...
        if(target ~= ply) then
            ...
            if Configbdrag.ShowDistance > distance and IsPedDeadOrDying(target, 0) then
                closestPlayer = value
				break
            end
        end
...
Citizen.CreateThread(function()
    while true do
		...
		if not IsPedDeadOrDying(p1) and not drag then
			...
			if GetDistanceBetweenCoords(p2pos, p1pos, true) <= Configbdrag.ShowDistance then
				if IsPedDeadOrDying(p2, 0) then
					if GetEntityHealth(p2) <= 6 then
						SetEntityInvincible(p2, true)
						if leojob[ESX.PlayerData.job.name] then
							if GetDistanceBetweenCoords(p2pos, p1pos, true) <= Configbdrag.InteractDistance then
								DrawText3D(p2pos.x, p2pos.y, p2pos.z, '~w~[~b~H~w~] Drag')
