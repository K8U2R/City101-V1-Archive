...
AddEventHandler("playerDropped", function()
	...
	for _, player in pairs(playersInCurrentRadioChannel) do
		--if player.Source ~= src then
			TriggerClientEvent("esx_radiolist:Client:SyncRadioChannelPlayers", player.Source, src, 0, playersInCurrentRadioChannel)
		--end
	end
...
RegisterNetEvent('esx_wesam:setNameInRadioAfterChangeJobPlayer')
AddEventHandler('esx_wesam:setNameInRadioAfterChangeJobPlayer', function()
	local xPlayer = Core.GetPlayerFromId(source)
	if xPlayer then
		if xPlayer.job.name == "admin" or xPlayer.job.name == "police" or xPlayer.job.name == "agent" or xPlayer.job.name == "ambulance" or xPlayer.job.name == "mechanic" then
			TriggerClientEvent('esx_radiolist:Client:ChangeNameInRadio', xPlayer.source, xPlayer.getJob().grade_label, xPlayer.getName())
		else
			TriggerClientEvent("esx_radiolist:Client:ChangeNameInRadio2", xPlayer.source, xPlayer.getName())
		end
	end
end)
...
AddEventHandler('pma-voice:setPlayerRadio', function(channelToJoin)
	...
	if radioChannelToJoin == 0 then
		...
		if xPlayer.job.name == "admin" or xPlayer.job.name == "police" or xPlayer.job.name == "agent" or xPlayer.job.name == "ambulance" or xPlayer.job.name == "mechanic" then
			TriggerClientEvent('esx_radiolist:Client:ChangeNameInRadio', src, xPlayer.getJob().grade_label, xPlayer.getName())
		end
	end
end)
...
function Connect(src, currentRadioChannel, radioChannelToJoin)
	...
	for _, player in pairs(playersInCurrentRadioChannel) do
		TriggerClientEvent("esx_radiolist:Client:SyncRadioChannelPlayers", player.Source, src, radioChannelToJoin, playersInCurrentRadioChannel)
	end
	playersInCurrentRadioChannel = {}
end
...
function Disconnect(src, currentRadioChannel) 
	local playersInCurrentRadioChannel = CreateFullRadioListOfChannel(currentRadioChannel)
	TriggerClientEvent("esx_radiolist:Client:SyncRadioChannelPlayers", src, src, 0, playersInCurrentRadioChannel)
	for _, player in pairs(playersInCurrentRadioChannel) do
		TriggerClientEvent("esx_radiolist:Client:SyncRadioChannelPlayers", player.Source, src, 0, playersInCurrentRadioChannel)
	end
	playersInCurrentRadioChannel = {}
