...
RegisterNUICallback("ban", function(data,cb)
    if Cooldown_count == 0 then 
		...
		ESX.TriggerServerCallback("el_bwh:ban",function(success,reason)
			if success then
				--ESX.ShowNotification("تم حظر الاعب من دخول السيرفر")
			else
				--ESX.ShowNotification(reason)
			end -- dont ask why i did it this way, im a bit retarded
		end, data.target, data.reason, data.length, data.offline)
	else
		ESX.ShowNotification('<font color=red>الرجاء عدم تدبيل الباند')
		ESX.ShowNotification('<font color=orange>أنتظر '..Cooldown_count..' ثواني')
	end
end)
...
RegisterNUICallback("warn", function(data,cb)
	...
	ESX.TriggerServerCallback("el_bwh:warn",function(success)
		if success then ESX.ShowNotification("تم إنذار اللاعب") else ESX.ShowNotification("حدث خطأ") end
...
RegisterNUICallback("unban", function(data,cb)
	...
	ESX.TriggerServerCallback("el_bwh:unban",function(success)
		if success then ESX.ShowNotification("تم إزالة الباند من اللاعب") else ESX.ShowNotification("ليس لديك صلاحيات كافية لفك الباند") end
	end, data.id)
	else
	ESX.ShowNotification('<font color=red>الرجاء عدم تدبيل فك الباند')
	ESX.ShowNotification('<font color=orange>أنتظر '..Cooldown_count..' ثواني')
	end
end)
...
AddEventHandler("el_bwh:gotBanned",function(rsn)
	Citizen.CreateThread(function()
		...
		PushScaleformMovieMethodParameterString("<FONT FACE='A9eelsh'>~r~ﺎﻴﺠﻳﺮﻴﻧ ﺔﻌﻃﺎﻘﻣ ﺮﻓﺮﻴﺳ ﻝﻮﺧﺩ ﻦﻣ ﻙﺮﻈﺣ ﻢﺗ")
