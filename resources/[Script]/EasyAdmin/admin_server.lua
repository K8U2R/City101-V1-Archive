...
ESX.RegisterServerCallback("esx_wesam:getbanslist", function(source, cb, id_banned)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer then
		if xPlayer.job.name == "admin" then
			local banslist = LoadResourceFile(GetCurrentResourceName(), "banlist.json")
			local obs = json.decode(banslist)
			local list = {}
			local have = false
			for i,theBan in pairs(obs) do
				local bannedidplayer = obs[i].banid
				if bannedidplayer == id_banned then
					have = true
					local name_player_has_been_banned = obs[i].name
					local name_admin = obs[i].banner
					local reasonbannedplayer = obs[i].reason
					cb({label = 'المتبند<br><span  style="color:#white;font-size:15"><span style="color:FF0000">' .. name_player_has_been_banned .. '</span><br><span  style="color:#white;font-size:15">الي مبندة</span><br><span  style="color:#orange;font-size:15">' .. name_admin .. '</span><br><span  style="color:#white;font-size:15">السبب</span><br><span  style="color:#white;font-size:15"><span style="color:orange">' .. reasonbannedplayer .. '</span><br><span  style="color:#white;font-size:15"><span style="color:white">رقم ايدي الباند</span><br><span  style="color:#white;font-size:15"><span style="color:FF0000">' .. bannedidplayer .. '</span>', banned_id = bannedidplayer})
				end
			end
			if not have then
				xPlayer.showNotification("<font color=red>لايوجد احد متبند بهاذا الأيدي</font>")
				cb(false)
			end
		else
			xPlayer.showNotification("<font color=red>صلاحيتك لاتسمح</font>")
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	RegisterServerEvent("EasyAdmin:banPlayer", function(playerId,reason,reason2,name_ban,expires)
		if playerId ~= nil then
			if (DoesPlayerHavePermission(source, "player.ban.temporary") or DoesPlayerHavePermission(source, "player.ban.permanent")) and CachedPlayers[playerId] then
				...
				TriggerClientEvent('chatMessage', -1, " ⭐ الرقابة و التفتيش   " ,  {198, 40, 40} ,  "حظر ^3" .. username .. "^0 من دخول السيرفر بسبب ^3" .. reason2 .. "^0 ^3" .. name_ban)
				...
				for ci,identifier in ipairs(blacklisted.identifiers) do
					if identifier == theId and matchingIdentifiers[theId] ~= true then
						...
						if matchingIdentifierCount >= minimumMatchingIdentifierCount then
							...
							deferrals.done(string.format( GetLocalisedText("bannedjoin"), blacklist[bi].banner, blacklist[bi].name, blacklist[bi].reason, formatDateString(blacklist[bi].expire), blacklist[bi].banid))
							return
						end
					end
				end
