		...
		if not mainMenu or not mainMenu:Visible() then
			...
			if strings and isAdmin then
				...
				if permissions["player.ban.permanent"] then
					table.insert(banLength, {label = "<FONT FACE='A9eelsh'>ﻲﺋﺎﻬﻧ ﺪﻧﺎﺑ", label2 = "نهائي", time = 10444633200})
				end

				if permissions["player.ban.temporary"] then
					table.insert(banLength, {label = "<FONT FACE='A9eelsh'>مﻮﻳ ﺪﻧﺎﺑ", label2 = "يوم", time = 86400})
					table.insert(banLength, {label = "<FONT FACE='A9eelsh'>مﻮﻳ ﺪﻧﺎﺑ", label2 = "يومين", time = 172800})
					table.insert(banLength, {label = "<FONT FACE='A9eelsh'>عﻮﺒﺳﺍ ﺪﻧﺎﺑ", label2 = "اسبوع", time = 518400})
					table.insert(banLength, {label = "<FONT FACE='A9eelsh'>ﻦﻴﻋﻮﺒﺳﺍ ﺪﻧﺎﺑ", label2 = "اسبوعين", time = 1123200})
					table.insert(banLength, {label = "<FONT FACE='A9eelsh'>ﺮﻬﺷ ﺪﻧﺎﺑ", label2 = "شهر", time = 2678400})
				end
				GenerateMenu()
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if RedM or tonumber(settings.button) then -- legacy watch menu button for press, for people that ignored the installation instructions, or use RedM.
			if (RedM and IsControlJustReleased(0, Controls[settings.button]) ) and (not RedM and IsControlJustReleased(0, tonumber(settings.button)) and GetLastInputMethod( 0 )) then
				...
				if strings and isAdmin then
					...
					if permissions["player.ban.temporary"] then
						table.insert(banLength, {label = "<FONT FACE='A9eelsh'>تﺎﻋﺎﺳ 6 ﺪﻧﺎﺑ", label2 = "6 ساعات", time = 21600})
						table.insert(banLength, {label = "<FONT FACE='A9eelsh'>تﺎﻋﺎﺳ 12 ﺪﻧﺎﺑ", label2 = "12 ساعة", time = 43200})
						table.insert(banLength, {label = "<FONT FACE='A9eelsh'>مﻮﻳ ﺪﻧﺎﺑ", label2 = "يوم", time = 86400})
						table.insert(banLength, {label = "<FONT FACE='A9eelsh'>مﺎﻳﺍ ثﻼﺛ ﺪﻧﺎﺑ", label2 = "ثلاث ايام", time = 259200})
						table.insert(banLength, {label = "<FONT FACE='A9eelsh'>عﻮﺒﺳﺍ ﺪﻧﺎﺑ", label2 = "اسبوع", time = 518400})
						table.insert(banLength, {label = "<FONT FACE='A9eelsh'>ﻦﻴﻋﻮﺒﺳﺍ ﺪﻧﺎﺑ", label2 = "اسبوعين", time = 1123200})
						table.insert(banLength, {label = "<FONT FACE='A9eelsh'>ﺮﻬﺷ ﺪﻧﺎﺑ", label2 = "شهر", time = 2678400})
						table.insert(banLength, {label = "<FONT FACE='A9eelsh'>ﺔﻨﺳ ﺪﻧﺎﺑ", label2 = "سنة", time = 31536000})
					end
...
function GenerateMenu() -- this is a big ass function
	...
	for i,thePlayer in pairs(players) do
		...
		if permissions["player.ban.temporary"] or permissions["player.ban.permanent"] then
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'reason_ban', {
					title = 'ادخل سبب الباند'
				}, function(data10, menu10)
					if data10.value == "" or data10.value == nil then
						ESX.ShowNotification("رجاء ادخل سبب الباند لايمكنك تبنيد شخص بدون سبب")
					else
						menu10.close()
						BanReason = data10.value
						BanReason2 = data10.value
						thisItem:RightLabel("<FONT FACE='A9eelsh'>" .. data10.value)
					end
			...
			thisItem.Activated = function(ParentMenu,SelectedItem)
				if BanReason == "" then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'reason_ban2', {
						title = 'ادخل سبب الباند'
					}, function(data11, menu11)
						if data11.value == "" or data11.value == nil then
							ESX.ShowNotification("رجاء ادخل سبب الباند لايمكنك تبنيد شخص بدون سبب")
						else
							menu11.close()
							BanReason = data11.value
							BanReason2 = data11.value
							thisItem:RightLabel("<FONT FACE='A9eelsh'>" .. BanReason)
						end
					end, function(data11, menu11)
						menu11.close()
				end)
				end
				TriggerServerEvent("EasyAdmin:banPlayer", thePlayer.id, BanReason, BanReason2, banLength[BanTime].label2, banLength[BanTime].time)
				BanTime = 1
				BanReason = ""
				_menuPool:CloseAllMenus()
				Citizen.Wait(800)
