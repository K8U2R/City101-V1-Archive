...
AddEventHandler("gksphone:jbmessage",function(name, number, message, photo, gps, jobm)
    ...
    if identifier ~= nil then
        ...
        for i=1, #Players1, 1 do
            local xPlayer2 = ESX.GetPlayerFromId(Players1[i])
            if xPlayer2.job.name == jobm then
                xPlayer2.showNotification("<font size=5 color=white><center><b>"..'<font color=yellow>رسالة جديدة من </font><font color=orange>'.."تطبيق الوظيفة"..'</font>'.."<br><br>"..message)
            end
        end
        if (json.encode(jobs) ~= '[]') then
            for i=1, #jobs, 1 do
                TriggerClientEvent('mesajjgetir', jobs[i].id, jobm)
                TriggerClientEvent('jobnotisound', jobs[i].id)
            end
            TriggerEvent('jobnotif', name, number, message, photo, jobm, identifier, 1)
        else
            TriggerClientEvent('notifi', source, {title = 'JOB notification', message = _U('not_player'), img= '/html/static/img/icons/jobm.png' })
        end
    end
end)
