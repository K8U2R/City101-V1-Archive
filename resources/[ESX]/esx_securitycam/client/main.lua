...
Citizen.CreateThread(function()
	while true do
		...
		for k, v in pairs(Config.Zones) do
			if PlayerData.job ~= nil and PlayerData.job.name ~= 'unemployed' and PlayerData.job.name == v.job then
				if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
					...
					--[[if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5 then
						DrawText3D(v.Pos.x, v.Pos.y, v.Pos.z+0.9, "~w~[~r~E~w~] Starta Camera", 0.50)
					end]]
				end
			end
		end
	...
	while true do
		...
		if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'cameras' then
			...
			if not menuopen then
				...
				local elements = {
					{label = 'البنك المركزي',   value = 'bankmenu'},
					{label = 'مركز الشرطة', value = 'policemenu'},
					{label = 'ميناء مدينة 101 البحري', value = 'seaportmenu'},
				}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
				{
					...
					elseif data.current.value == 'seaportmenu' then
						...
						if policeHacked then
							...
							seeportCameras = true
							blockbuttons = true
							local firstCamx = Config.Locations[1].seeportCameras[1].x
							local firstCamy = Config.Locations[1].seeportCameras[1].y
							local firstCamz = Config.Locations[1].seeportCameras[1].z
							local firstCamr = Config.Locations[1].seeportCameras[1].r
							...
							SendNUIMessage({
								type  = "enablecam",
								label = Config.Locations[1].seeportCameras[1].label,
								box   = Config.Locations[1].seeportCamLabel.label
							})
		...
		if createdCamera ~= 0 then
			...
			if IsControlJustPressed(0, Keys["LEFT"]) then
				...
				elseif seeportCameras then
					local newCamIndex

					if currentCameraIndexIndex == 1 then
						newCamIndex = #Config.Locations[currentCameraIndex].seeportCameras
					else
						newCamIndex = currentCameraIndexIndex - 1
					end

					local newCamx = Config.Locations[currentCameraIndex].seeportCameras[newCamIndex].x
					local newCamy = Config.Locations[currentCameraIndex].seeportCameras[newCamIndex].y
					local newCamz = Config.Locations[currentCameraIndex].seeportCameras[newCamIndex].z
					local newCamr = Config.Locations[currentCameraIndex].seeportCameras[newCamIndex].r

					SetFocusArea(newCamx, newCamy, newCamz, newCamx, newCamy, newCamz)
					SendNUIMessage({
						type = "updatecam",
						label = Config.Locations[currentCameraIndex].seeportCameras[newCamIndex].label
					})
			...
			if IsControlJustPressed(0, Keys["RIGHT"]) then
				...
				elseif seeportCameras then
					...
					if currentCameraIndexIndex == #Config.Locations[currentCameraIndex].seeportCameras then
						newCamIndex = 1
					else
						newCamIndex = currentCameraIndexIndex + 1
					end
...
AddEventHandler('esx_securitycam:unhackanim', function()
	...
	while (not HasAnimDictLoaded("anim@heists@humane_labs@emp@hack_door")) do Citizen.Wait(100) end
