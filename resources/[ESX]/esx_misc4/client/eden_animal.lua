...
function OpenPetMenu()
	...
	if come == 1 then
		...
		table.insert(elements, {label = '<span style="color:gray">الجــوع: </span> '..status..'', value = nil})
		table.insert(elements, {label = 'اطعام', value = 'graille'})
		table.insert(elements, {label = 'تقيد او فك قيد', value = 'attached_animal'})

		if isInVehicle then
			table.insert(elements, {label = 'اخرج من السيارة', value = 'vehicle'})
		else
			table.insert(elements, {label = 'اصعد المركبة', value = 'vehicle'})
		end

		table.insert(elements, {label = 'اصدر اوامر', value = 'give_orders'})

	else
		table.insert(elements, {label = '<span style="color:00EE4F">نداء الحيوان الأليف</br><span style="color:yellow; font-size:15">يمكن شراء حيوانك الأليف المفضل من المحل المخصص بالريف</span>', value = 'come_animal'})
		if ESX.PlayerData.job.name == "police" or ESX.PlayerData.job.name == "agent" then

			table.insert(elements, {label = '<span style="color:00EE4F">نداء ال <span style="color:3399FF">K9</br><span style="color:yellow; font-size:15">هذا الحيوان مخصص فقط لأدارة الشرطة و حرس الحدود</span>', value = 'police_k9'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pet_menu', {
		title    = 'التعامل مع الحيوان الأليف',
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		if data.current.value == 'come_animal' and come == 0 then
			...
			menu.close()
		elseif data.current.value == "police_k9" and come == 0 then
		...
		elseif data.current.value == 'attached_animal' then
			if not IsPedSittingInAnyVehicle(ped) then
				...
				ESX.ShowNotification('لا يمكنك إرفاق حيوانك الأليف في مركبة')
			end
		elseif data.current.value == 'give_orders' then
			GivePetOrders()
		elseif data.current.value == 'graille' then
			local inventory = ESXDATA.inventory
			...
			if distance < 5 then
				if count >= 1 then
					if status < 100 then
						...
						ESX.ShowNotification('لم يعد حيوانك الأليف جائعًا')
					end
				else
					ESX.ShowNotification('ليس لديك طعام')
				end
			else
				ESX.ShowNotification('حيوانك الأليف بعيد جدًا')
			end
			...
			if not isInVehicle then
				if IsPedSittingInAnyVehicle(playerPed) then
					if distance < 8 then
						...
						ESX.ShowNotification('حيوانك الأليف بعيد جدًا عن السيارة')
					end

				else
					ESX.ShowNotification('عليك أن تكون في سيارة')
				end
			else
				if not IsPedSittingInAnyVehicle(playerPed) then
					...
					ESX.ShowNotification('ما زلت في السيارة')
				end
			end
		end
...
function GivePetOrders()
	ESX.TriggerServerCallback('eden_animal:getPet', function(pet)
		...
		if not inanimation then
			if pet ~= 'chat' then
				table.insert(elements, {label = 'احضر الكره', value = 'balle'})
			end

			table.insert(elements, {label = 'اذهب عند قدمي',     value = 'pied'})
			table.insert(elements, {label = '<span style="color:FF0E0E">امر الذهاب إلى البيت</span>', value = 'return_doghouse'})

			if pet == 'chien' then
				table.insert(elements, {label = 'الجلوس', value = 'assis'})
				table.insert(elements, {label = 'نام', value = 'coucher'})
			elseif pet == 'chat' then
				table.insert(elements, {label = 'نام', value = 'coucher2'})
			elseif pet == 'loup' then
				table.insert(elements, {label = 'نام', value = 'coucher3'})
			elseif pet == 'carlin' then
				table.insert(elements, {label = 'الجلوس', value = 'assis2'})
			elseif pet == 'retriever' then
				table.insert(elements, {label = 'الجلوس', value = 'assis3'})
			elseif pet == 'rottweiler' then
				table.insert(elements, {label = 'الجلوس', value = 'assis4'})
			end
		else
			table.insert(elements, {label = 'قوم', value = 'debout'})
		end

		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_orders', {
			title    = 'أوامر الحيوانات الأليفة',
			align    = 'bottom-right',
			elements = elements
		}, function(data, menu)
			if data.current.value == 'return_doghouse' then
				...
				ESX.ShowNotification('حيوانك الأليف يعود للمنزل')
			...
			elseif data.current.value == 'balle' then
				...
				if DoesEntityExist(object) then
					...
					ESX.ShowNotification('لا يوجد كرات')
				end
...
Citizen.CreateThread(function()
	while true do
		...
		if status == 0 then
			...
			DeleteEntity(ped)
			ESX.ShowNotification('حيوانك الأليف مات! أعطه الطعام في المرة القادمة')
			come = 3
			status = "die"
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(Config.Zones) do
			if GetDistanceBetweenCoords(pedCoords, v.Pos.x, v.Pos.y, v.Pos.z, true) <= 10.0 then
				letsleep = false
...
Citizen.CreateThread(function()
	...
	AddTextComponentString('<font face="A9eelsh">ﺕﺎﻧﺍﻮﻴﺤﻟﺍ ﺮﺠﺘﻣ')
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coord, Config.Zones.PetShop.Pos.x, Config.Zones.PetShop.Pos.y, Config.Zones.PetShop.Pos.z, true) < 2 then
			ESX.ShowHelpNotification('press ~INPUT_CONTEXT~ to access the pet shop.')
...
function OpenPetShop()
	...
	for i=1, #Config.PetShop, 1 do
		table.insert(elements, {
			label = ('%s - <span style="color:green;">%s</span>'):format(Config.PetShop[i].label, '$'..ESX.Math.GroupDigits(Config.PetShop[i].price)),
