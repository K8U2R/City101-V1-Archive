...
RegisterCommand('nearby', function(source, args, raw)
    ...
    if(distance ~= -1 and distance < 3) then
        if DP.Emotes[emotename] ~= nil then
            dict, anim, ename = table.unpack(DP.Emotes[emotename])
            TriggerServerEvent("esx_animations:ServerEmoteRequest", GetPlayerServerId(target), emotename)
        else
            EmoteChatMessage("'"..emotename.."' "..Config.Languages[lang]['notvalidsharedemote'].."")
        end
    else
        --SimpleNotify(Config.Languages[lang]['nobodyclose'])
        ESX.ShowNotification('<font color=red>تم رفض الحركة')
    end
end, false)

RegisterNetEvent("esx_animations:SyncPlayEmote")
AddEventHandler("esx_animations:SyncPlayEmote", function(emote, player)
    EmoteCancel()
    Wait(300)
...
RegisterNetEvent("esx_animations:SyncPlayEmoteSource")
AddEventHandler("esx_animations:SyncPlayEmoteSource", function(emote, player)
...
RegisterNetEvent("esx_animations:ClientEmoteRequestReceive")
AddEventHandler("esx_animations:ClientEmoteRequestReceive", function(emotename, etype)
    isRequestAnim = true
    ...
    ESX.ShowNotification('<font color=orange>طلب تمثيل حركة</font></br>'..remote..'</br><font color=yellow>للموافقة اضغط Y')
    ...
    while true do
        ...
        if IsControlJustPressed(0, 246) and isRequestAnim then
            ...
            if(distance ~= -1 and distance < 3) then
                ...
                TriggerServerEvent("esx_animations:ServerValidEmote", GetPlayerServerId(target), requestedemote, otheremote)
                isRequestAnim = false
            else
                --SimpleNotify(Config.Languages[lang]['nobodyclose'])
        ...
        elseif IsControlJustPressed(0, 182) and isRequestAnim then
			...
			ESX.ShowNotification('<font color=red>اللاعب رفض الحركة')
            isRequestAnim = false
        end
    end
end)
