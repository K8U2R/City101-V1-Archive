...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("<FONT FACE='sharlock'>ﺹﺎﺻﺭ ﻊﻣ ﺱﺪﺴﻣ ﻰﻟﺍ ﺔﺟﺎﺤﺑ ﺖﻧﺃ")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification("<FONT FACE='sharlock'>ﻚﻨﻣ ﺏﺮﻘﻟﺎﺑ ﺐﻋﻻ ﺪﺟﻮﻳﻻ")
		end 
	end
...
AddEventHandler('cmg3_animations:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach,KillerIdentifier)
	...
	if animation2 == "victim_fail" then 
		...
		DetachEntity(GetPlayerPed(-1), true, false)
		ESX.ShowNotification('رقم القاتل : <br><center><font color=red>'..KillerIdentifier..'</font></center>')
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"ﺔﻨﻴﻫﺮﻟﺍ ﻞﺘﻗ [~r~H~w~] - [~g~G~w~] ﺔﻨﻴﻫﺮﻟﺍ ﺡﺍﺮﺳ ﻕﻼﻃﺇ")
