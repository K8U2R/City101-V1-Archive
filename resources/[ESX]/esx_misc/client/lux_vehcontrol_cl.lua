...
{	
	...
	"UNMARKED16",
	"UNMARKED17",
	"gruppe8", --دورية حراسات سري
	"gruppe9", --دورية حراسات سري
	"gruppe10", --دورية حراسات سري
	"uparmorhmvdes", --army
	"hmmwv", --army
	...
	"tailgater2",
	"FIBJackal",
	"police4", --دورية مباحث - تاكسي
	"sspres",
	
	--other
	"riot3", --مدرعة حراسات - حجم كبير
	"gruppe6", --باص السجن المركزي - حرس الحدود
	"tfft", --مكافحة حريق - القوات المسلحة
	"pol2", --شاحنة عمليات متنقلة 2
	"towtruck6", --ونش مطافي
}

local eModelsWithBikecall =
{	
	"policeb", --دورية دراجة نارية - المرور
	"polkuwb1", --دراجة شرطة
	"gruppe5", --دورية حراسات - دراجة نارية
	"pol1", --دورية حراسات - دراجة نارية
}
...
function useFIBSrn(veh)
	local model = GetEntityModel(veh)
	for i = 1, #eModelsWithFIBcall, 1 do
		if model == GetHashKey(eModelsWithFIBcall[i]) then
			return true
		end
	end
	return false
...
-----------------------------------------------------------------------
function useBikeSrn(veh)
	local model = GetEntityModel(veh)
	for i = 1, #eModelsWithBikecall, 1 do
		if model == GetHashKey(eModelsWithBikecall[i]) then
			return true
		end
	end
	return false
	...
	if DoesEntityExist(veh) and not IsEntityDead(veh) then
		if newstate ~= state_lxsiren[veh] then
			...
			elseif newstate == 3 then
				snd_lxsiren[veh] = GetSoundId()
				if useFiretruckSiren(veh) or useFIBSrn(veh) then
