...
function openmenuvehicle()
  ...
  myVeh = false
  if Config.blacklisted_model[GetEntityModel(vehicle)] then
	  myVeh = true
	end
  ...
  if not Config.CheckOwnership or (Config.AllowPolice and ESX.PlayerData.job.name == 'police') or (Config.CheckOwnership and myVeh) or (Config.AllowPolice and ESX.PlayerData.job.name == 'agent') or (Config.AllowPolice and ESX.PlayerData.job.name == 'admin') then
      ...
      if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
        ...
        if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
          ...
          if locked == 1 or class == 15 or class == 16 or class == 14 or class == 19 or ESX.PlayerData.job.name == "admin" or ESX.PlayerData.job.name == "police" or ESX.PlayerData.job.name == "agent" then
...
function OpenCoffreInventoryMenu(plate,max)
  local is_job = false
  ESX.TriggerServerCallback('esx_owner_car:getOwnerCarJob', function(true_or_false)
    ESX.TriggerServerCallback('esx_owner_car:getOwnerCar', function(true_no_false)
      if true_no_false or true_or_false or ESX.PlayerData.job.name == "police" or ESX.PlayerData.job.name == "admin"  or ESX.PlayerData.job.name == "agent" then
        ESX.TriggerServerCallback('esx_trunk:getInventoryV', function(inventory)
          local plate = plate
          local owner= GetPlayerPed(-1)
          local elements = {}
          table.insert(elements, {label = '<span style="color:00EE4F">إيداع</span>', type = 'deposer', value = 'deposer'})
          table.insert(elements, {label = 'اموال غير شرعية <span style="color:FF0E0E">$</span>' .. inventory.blackMoney, type = 'item_account', value = 'black_money'})
    
          for i=1, #inventory.items, 1 do
            local item = inventory.items[i]
            if item.count > 0 then
              table.insert(elements, {label = item.label .. ' x' .. item.count..' - (<span style="color:FFAE00">'.. ((getItemyWeight(item.name)*item.count)/1000) ..' '.._U('measurement')..')', type = 'item_standard', value = item.name})
            end
          ...
          for i=1, #inventory.weapons, 1 do
            ...
            table.insert(elements, {label = ESX.GetWeaponLabel(weapon.name) .. ' [' .. weapon.ammo .. '] - (<span style="color:FFAE00">'..(getItemyWeight(weapon.name)/1000)..' '.._U('measurement')..')', type = 'item_weapon', value = weapon.name, ammo = weapon.ammo})
          end
    
          ESX.UI.Menu.Open(
            'default', GetCurrentResourceName(), 'car_inventory',
            {
              title    ='<span style="color:FFAE00">'.. plate .. '</span>'..' - ' .. (inventory.weight/1000) .. " / "..(max/1000).._U('measurement'),
              align    = 'top-left',
              elements = elements,
            },
            function(data, menu)
        ...
        ESX.ShowNotification('ليست مركبتك')
      end
    end, plate)
  end, plate)
end
...
function OpenPlayerInventoryMenu(owner,plate,max,weight)
  ...
  ESX.TriggerServerCallback('esx_trunk:getPlayerInventory', function(inventory)
    ...
    local elements = {}
    table.insert(elements, {label = '<span style="color:FF0E0E">رجوع للقائمة السابقة</span>', type = 'retour', value = 'retour'})
    table.insert(elements, {label = 'اموال غير شرعية <span style="color:FF0E0E">$</span>' .. inventory.blackMoney, type = 'item_account', value = 'black_money'})
    ...
    for i=1, #inventory.items, 1 do
      ...
      if item.count > 0 then
        table.insert(elements, {label = item.label .. ' x' .. item.count..' - (<span style="color:FFAE00">'.. ((getItemyWeight(item.name)*item.count)/1000) ..' Kg</span)', type = 'item_standard', value = item.name})
      end
    ...
    for i=1, #weaponList, 1 do
      ...
      if HasPedGotWeapon(playerPed,  weaponHash,  false) and weaponList[i].name ~= 'WEAPON_UNARMED' then
        ...
        table.insert(elements, {label = weaponList[i].label .. ' [' .. ammo .. '] -  (<span style="color:FFAE00">'..(getItemyWeight(weaponList[i].name)/1000)..' '..' Kg</span>)', type = 'item_weapon', value = weaponList[i].name, ammo = ammo})
      end
