...
AddEventHandler('onMySQLReady', function()
  local result = MySQL.Sync.fetchAll('SELECT plate, owned, data FROM trunk_inventory')
  local data = nil
  if #result ~= 0 then
    for i=1,#result,1 do
...
function loadInvent(plate)
  local result = MySQL.Sync.fetchAll('SELECT plate, owned, data FROM trunk_inventory WHERE plate = @plate',
  {
    ['@plate'] = plate,
  })
  local data = nil
...
function getOwnedVehicule(plate)
  ...
  if not found then
    local result = MySQL.Sync.fetchAll('SELECT vehicle FROM owned_vehicles')
    while result == nil do
      Wait(5)
    end
