...
ESX.RegisterServerCallback('esx_trunk:getWightVehicle', function(source, cb, plate)
  ...
  MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE plate = @plate", {
    ...
    if result2[1] then
      ...
      MySQL.Async.fetchAll("SELECT * FROM owned_veh_job WHERE plate = @plate AND identifier = @identifier", {
        ["@plate"] = plate,
        ['@identifier'] = xPlayer.identifier
      },function(result)
        if result[1] ~= nil then
          cb(result[1].name_car)
        else
          if xPlayer.job.name == "admin" or xPlayer.job.name == "police" or xPlayer.job.name == "agent" then
            MySQL.Async.fetchAll("SELECT * FROM owned_veh_job WHERE plate = @plate", {
              ["@plate"] = plate,
            },function(result)
              if result[1] ~= nil then
                cb(result[1].name_car)
