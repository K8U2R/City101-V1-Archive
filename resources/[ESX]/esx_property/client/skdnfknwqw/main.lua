...
function CreateBlips()
	for i=1, #Config.Properties, 1 do
		...
		if property.entering then
			...
			SetBlipColour(Blips[property.name], 3)
...
function SetPropertyOwned(name, owned, rented)
	...
	if owned then
		...
		SetBlipColour(Blips[enteringName], 5)
		SetBlipScale  (Blips[property.name], 0.8)
		BeginTextCommandSetBlipName("STRING")
...
function OpenRoomMenu(property, owner, ide)
	local entering = nil
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room', {
		...
		elseif data.current.value == 'room_inventory' then
			OpenRoomInventoryMenu(property, owner, ide)
		elseif data.current.value == 'player_inventory' then
			OpenPlayerInventoryMenu(property, owner, ide)
		end
...
function OpenRoomInventoryMenu(property, owner, iden)
...
function OpenPlayerInventoryMenu(property, owner, ident)
	ESX.TriggerServerCallback('esx_property:getPlayerInventory', function(inventory)
		...
		for i=1, #inventory.items, 1 do
			local item = inventory.items[i]
			local can_add_item = true
			for k,v in pairs(Config.items_block) do
				if Config.items_block[k].item == item.name then
					can_add_item = false
				end
			end
			if can_add_item then
				if item.count > 0 then
					table.insert(elements, {
						label = item.label .. ' x' .. item.count,
						type  = 'item_standard',
		...
		for i=1, #Config.Properties, 1 do
			...
			if property.entering and not property.disabled then
				...
				if distance < Config.DrawDistance then
					...
					ESX.Game.Utils.DrawText3D(property.entering, '', 2)
					letSleep = false
				end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'room_menu' then
					OpenRoomMenu(CurrentActionData.property, CurrentActionData.owner, ESX.GetPlayerData().identifier)
