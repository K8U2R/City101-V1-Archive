...
function init()
	if Settinggs.VerboseServer then
		print("> > > S A L T Y _ T O K E N I Z E R  < < <")
	end
	math.randomseed(os.time())
	TriggerEvent('Roma_security:serverReady')
end
...
function generateToken()
	math.randomseed(os.time())
	local token = string.random(Settinggs.TokenLength, Settinggs.TokenCharset)
	while not isTokenUnique(token) do
		token = string.random(Settinggs.TokenLength, Settinggs.TokenCharset)
	end
	return string.random(Settinggs.TokenLength, Settinggs.TokenCharset)
end
...
function setupServerResource(resource)
	...
	AddEventHandler('Roma_security:playerLoaded', function(player)
...
function secureServerEvent(resource, player, token)
	...
	elseif _source == "" then -- If the request came from the server, then no need to authenticate the token
		...
		if token ~= resourceTokens[resource] then
			...
			if Settinggs.CustomAction then
				Settinggs.CustomActionFunction(_source)
			else
				DropPlayer(_source, Settinggs.KickMessage)
			end
			return false
		end
	end
...
RegisterNetEvent('Roma_security:requestObfuscation')
AddEventHandler('Roma_security:requestObfuscation', function(resourceName, id)
	local _source = source
	TriggerClientEvent('Roma_security:obfuscateReceived', _source, id, getObfuscatedEvent(_source, resourceName))
end)

RegisterNetEvent('Roma_security:playerSpawned')
AddEventHandler('Roma_security:playerSpawned', function()
	local _source = source
	initComplete = true
	...
	if not didPlayerLoad[_source] then
		...
		TriggerEvent('Roma_security:playerLoaded', _source)
	else
