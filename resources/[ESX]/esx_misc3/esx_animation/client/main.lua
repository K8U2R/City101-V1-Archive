			  ...
			  elseif type == 'dpemotes' then
				  ...
				  if emotesType == 'Shared' then
					  ...
					  if closestPlayer ~= -1 and closestDistance <= 1.5 then
						  TriggerServerEvent('esx_animations:ServerEmoteRequest', GetPlayerServerId(closestPlayer),emotesName,data.current.label)
					  else
				  ...
				  elseif emotesType == 'Walks' then
                    local x = table.unpack(DP.Walks[emotesName])
                    RequestWalking(x)
                    SetPedMovementClipset(PlayerPedId(), x, 0.2)
                    RemoveAnimSet(x)
                    ESX.UI.Menu.Open(
                        'default', GetCurrentResourceName(), 'animations_sub_walk',
                        {
                            title    = 'استعراض طريقة المشي',
                            align    = 'top-left',
                            elements = {
                                { label = '<font color=green>حفظ طريقة المشي</font>', value = 'save' },
                                { label = '<font color=red>رجوع</font>', value = 'no' },
                            },
                        }, function(data3, menu3)
                            if data3.current.value == 'save' then
                                if Timing2 == 0 then
                                    savewalk = true
                                    WalkMenuStart(emotesName)
                                    Timing2 = 60
                                else
                                    ESX.ShowNotification('<font color=red> عليك الانتظار </font><font color=orange>'..Timing2..'</font> ثانية ')
                                end
	...
	while true do
		...
		if IsControlJustReleased(0, Keys['F3']) and not isDead then
            ...
            TriggerEvent('esx_status:getStatus', 'drunk', function(status)
                TriggerEvent('esx_status:getStatus', 'drug', function(status2)
                    if status.val == 0 then
                        if status2.val == 0 then
                            OpenAnimationsMenu()
                        else
                            ESX.ShowNotification('<font color=red>لا يمكن في حالة تعاطي</font>')
                        end
                    else
                        ESX.ShowNotification('<font color=red>لا يمكن في حالة سكر</font>')
                    end
                end)
            end)
