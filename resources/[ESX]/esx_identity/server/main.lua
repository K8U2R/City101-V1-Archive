...
elseif not Config.UseDeferrals then
	...
	AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
		if alreadyRegistered[xPlayer.identifier] == true then
			...
			TriggerClientEvent('chatMessage', -1, '', { 255, 255, 255 }, '^2 تسجيل دخول |^7 ' ..currentIdentity.firstName .. ' ' .. currentIdentity.lastName..' ')
			Citizen.Wait(30000)
			TriggerClientEvent('esx_adminjob:givePlayerMenuAfterJoin', xPlayer.source)
			playerIdentity[xPlayer.identifier] = nil
		else
			local src = source
			...
			TriggerClientEvent('chatMessage', -1, '', { 255, 255, 255 }, '^2 تسجيل دخول | ^3جديد او من غير هوية^7 |^7 ' ..GetPlayerName(playerId)..'')
		end
	end)
	...
	ESX.RegisterServerCallback('esx_identity:registerIdentity', function(source, cb, data)
		...
		if xPlayer then
			if not alreadyRegistered[xPlayer.identifier] then
				...
				playerIdentity[xPlayer.identifier] = nil
				cb(true)
				TriggerEvent('esx_identity:GetMeRpName', Config.pas)
			else
				cb(false)
			end
		end
...
ESX.RegisterServerCallback("esx_identity:ChangeIdentityMenu", function(source, cb)
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src) 
	local money = xPlayer.getAccount("money").money
   if money >= Config.ChangeIdentityPrice then
   xPlayer.removeMoney(Config.ChangeIdentityPrice)
   ----------------------------------------------
...
RegisterServerEvent('esx_identity:GetMeRpName')
AddEventHandler('esx_identity:GetMeRpName', function(pas)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer then
    	local yourName = xPlayer.getName()
		if pas==Config.pas then
			TriggerClientEvent('esx_identity:RpName', source, yourName)
		end
	end
end)
