...
AddEventHandler("TakeHostage:sync", function(targetSrc)
	...
	TriggerEvent("send:discordLogs", source, "قام بتهديد رهينة"..GetPlayerName(source).."", "#3AF241 ", "akra")
end)
...
AddEventHandler("TakeHostage:releaseHostage", function(targetSrc)
	...
	if takenHostage[targetSrc] then 
		...
		TriggerEvent("send:discordLogs", source, " قام بترك الرهينة "..GetPlayerName(source).."", "#3AF241 ", "akra")
	end
end)
...
AddEventHandler("TakeHostage:killHostage", function(targetSrc)
	...
	if takenHostage[targetSrc] then 
		...
		takingHostage[source] = nil
		takenHostage[targetSrc] = nil
		TriggerEvent("send:discordLogs", source, " قام بقتل الرهينة  "..GetPlayerName(source).."", "#3AF241 ", "akra")
	end
end)
...
AddEventHandler("TakeHostage:stop", function(targetSrc)
	...
	elseif takenHostage[source] then
		...
		TriggerEvent("send:discordLogs", source, " قام بأيقاف الحركة  "..GetPlayerName(source).."", "#3AF241 ", "akra")
	end
end)
