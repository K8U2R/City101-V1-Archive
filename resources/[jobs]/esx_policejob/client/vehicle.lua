				...
				if #jobVehicles > 0 then
					...
					for k,v in ipairs(jobVehicles) do
						...
						if IsModelInCdimage(props.model) then
							...
							local label = ('<span style="color:red;">%s</span> - <span style="color:white;">%s</span> - '):format(props.plate, v.name)
					...
					if #garage > 0 then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_garage', {
							...
							if data2.current.stored then
								local ModelUsed = nil
								for k,v in ipairs(Config.CarsPolice) do
									if v.name == data2.current.name_car then
										ModelUsed = v.model
									end
								end
								if ModelUsed == nil then
								...
								local foundSpawn, spawnPoint = GetAvailableVehicleSpawnPoint(station, part, partNum)
								local LiveryNumber = 4
								if ModelUsed == "doreat3" or ModelUsed == "doreat11" or ModelUsed == "doreat10" or ModelUsed == "doreat8" or ModelUsed == "doreat5" or ModelUsed == "doreat2" or ModelUsed == "doreat3" or ModelUsed == "doreatb1" then
									menu2.close()
									ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'choice_livery', {
										title = _U('garage_title'),
										align = 'top-left',
										elements = {
											{label = data2.current.name_car .. ' | <span style="color:#00FFFF">دورية أمنية</span>', value = "amneh"},
											{label = data2.current.name_car .. ' | <font color=gray>دورية إدارة الشرطة</span>', value == "amn_3am"}
										}
									}, function(data10, menu10)
										if data10.current.value == "amneh" then
											LiveryNumber = 3
										else
											if ModelUsed == "doreat8" or ModelUsed == "doreat11" then
												LiveryNumber = 5
											elseif ModelUsed == "doreat3" or ModelUsed == "doreat10" or ModelUsed == "doreat5" or ModelUsed == "doreat2" or ModelUsed == "doreatb1" then
												LiveryNumber = 4
											end
										end
										if foundSpawn then
