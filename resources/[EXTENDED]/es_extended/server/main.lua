	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT accounts, job, job_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', {
			...
			if result[1].position and result[1].position ~= '' then
				...
				userData.coords = {x = -1095.83, y = -318.94, z = 37.67, heading = 352.78}
			end
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount, pas)
    if(not pas==abdurhman.pas)then return end
	local playerId = source
	...
	elseif type == 'item_account' then
		if itemCount <= 5000 and sourceXPlayer.getAccount(itemName).money >= itemCount or sourceXPlayer.job.name == "admin" and sourceXPlayer.getAccount(itemName).money >= itemCount or sourceXPlayer.job.name == "police" and sourceXPlayer.job.grade >= 17 and sourceXPlayer.getAccount(itemName).money >= itemCount or sourceXPlayer.job.name == "agent" and sourceXPlayer.job.grade >= 15 and sourceXPlayer.getAccount(itemName).money >= itemCount or sourceXPlayer.job.name == "ambulance" and sourceXPlayer.job.grade >= 11 and sourceXPlayer.getAccount(itemName).money >= itemCount or sourceXPlayer.job.name == "mechanic" and sourceXPlayer.job.grade >= 11 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			if itemName == "black_money" then
				if itemCount > 5000 then
					...
					TriggerEvent("esx:givemoneyLog","wesamaskcbasjhcbasjsacascapasswordf",sourceXPlayer.source,targetXPlayer.source,itemName,itemCount)
...
AddEventHandler('esx:removeInventoryItem', function(type, itemName, itemCount, o, coords, pas) -- o = logthis
    if(not pas==abdurhman.pas)then return end
	local playerId = source
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				if o then
				TriggerEvent("esx:dropitemLog", 'kn29ndKK2an290', playerId, type, itemName, itemCount, coords)
				end
			end
		end
	elseif type == 'item_account' then
...
AddEventHandler('esx:useItem', function(itemName, pas)
    if(not pas==abdurhman.pas)then return end
	local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('esx:onPickup', function(pickupId, coords, pas)
    if(not pas==abdurhman.pas)then return end
	...
	if pickup then
		...
		if success then
		    TriggerEvent("esx:onPickupLog", 'Kj27bvKKuwp2', xPlayer.source, pickup.type, pickup.name, pickup.count, coords)
