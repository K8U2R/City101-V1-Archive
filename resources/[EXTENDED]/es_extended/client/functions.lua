...
--test2 pNotify
ESX.ShowNotification = function(msg, time)	
    if time ~= nil and time >= 1 then
	TriggerEvent("pNotify:SendNotification", {
		text = "<font size=5 color=white><center><b>"..msg,
		type = 'alert',
		queue = left,
		timeout = time,
		killer = false,
...
ESX.showpNotifyNotification = function(msg)	
	TriggerEvent("pNotify:SendNotification", {
		text = "<font size=5 color=white><center><b>"..msg,
		type = 'alert',
		queue = left,
...
ESX.ShowHelpNotification = function(msg)
	--if not IsHelpMessageBeingDisplayed() then
		...
		AddTextComponentSubstringPlayerName('<FONT FACE="A9eelsh">'..msg)
...
ESX.ShowInventory = function()
	...
	table.insert(elements, {label = "<span style='color:FF99CC;'>البطاقات الشخصية</span>", value = "id_cards"})
		...
		if v.money > 0 then
			...
			if v.name == "black_money" then
				table.insert(elements, {
					label = v.label .. "<span style='color:gray;'>:</span> " .. ESX.Math.GroupDigits(v.money) .. '<span style="color:FF0E0E;">$</span>',
					count = v.money,
					type = 'item_account',
					value = v.name,
					usable = false,
				...
				table.insert(elements, {
					label = v.label .. "<span style='color:gray;'>:</span> " .. ESX.Math.GroupDigits(v.money) .. '<span style="color:green;">$</span>',
					count = v.money,
					type = 'item_account',
					value = v.name,
					usable = false,
	...
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.count > 0 then
			...
			table.insert(elements, {
				label = v.label .. ' <span style="color:gray;">x</span><span style="color:orange;">' .. v.count .. "</span>",
				count = v.count,
				type = 'item_standard',
				value = v.name,
				usable = v.usable,
	...
	for k,v in ipairs(Config.Weapons) do
		...
		if HasPedGotWeapon(playerPed, weaponHash, false) then
			...
			if v.ammo then
				label = v.label .. ' <span style="color:gray;">x</span><span style="color:orange;">' .. ammo .. '</span> <span style="color:gray;">' .. v.ammo.label .. "</span>"
			else
				label = '<span style="color:gray;">' .. v.label .. '</span>'
			end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory', {
		...
		if data.current.value == "id_cards" then
			TriggerEvent("esx_accessories:openIDCARDSmenu")
		else
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item', {
								...
								if playersNearby[selectedPlayer] then
									...
									if IsPedOnFoot(selectedPlayerPed) and not IsPedFalling(selectedPlayerPed) then
										if type == 'item_weapon' then
											ESX.ShowNotification("لايمكنك اعطاء الأسلحة")
										else
											ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
												...
												if quantity and quantity > 0 and data.current.count >= quantity then
													if Cooldown_count == 0 then
														TriggerServerEvent('esx:giveInventoryItem', selectedPlayerId, type, item, quantity, abdurhman.pas)
														Cooldown(10)
														menu3.close()
														menu2.close()
														menu1.close()
													else
														ESX.ShowNotification(_U('cooldown_wait', Cooldown_count))
													end
												else
				...
				elseif data1.current.action == 'remove' then
					...
					if Cooldown_count == 0 then
					if not abdurhman.CanDropWeapons and type == 'item_weapon' then ESX.ShowNotification(_U('CantDropWeapons')) return end
						...
						if type == 'item_weapon' then
							...
							Citizen.Wait(1000)
							TriggerServerEvent('esx:removeInventoryItem', type, item, 1, true, GetEntityCoords(PlayerPedId()), abdurhman.pas)
							Cooldown(abdurhman.DropWaitTime)
						else
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_remove', {
								...
								if quantity and quantity > 0 and data.current.count >= quantity then
								if type == 'item_account' then if quantity > abdurhman.MaxMoneyDrop then ESX.ShowNotification(_U('MaxMoneyDrop', abdurhman.MaxMoneyDrop)) return end end
									menu2.close()
									menu1.close()
									TaskPlayAnim(playerPed, dict, anim, 8.0, 1.0, 1000, 16, 0.0, false, false, false)
									Citizen.Wait(1000)
									TriggerServerEvent('esx:removeInventoryItem', type, item, quantity, true, GetEntityCoords(PlayerPedId()), abdurhman.pas)
									Cooldown(abdurhman.DropWaitTime)
								else
									ESX.ShowNotification(_U('amount_invalid'))
								end
				...
				if Cooldown_count == 0 then 
				Cooldown(abdurhman.useItem_WaitTime)
					TriggerServerEvent('esx:useItem', item, abdurhman.pas)
				else
					ESX.ShowNotification(_U('cooldown_wait', Cooldown_count))
				end
								...
								ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
									...
									if quantity and quantity > 0 then
										if pedAmmo >= quantity then
											TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(closestPlayer), 'item_ammo', item, quantity, abdurhman.pas)
											menu2.close()
											menu1.close()
										else
...
RegisterNetEvent('esx:showpNotifyNotification')
AddEventHandler('esx:showpNotifyNotification', function(msg)
	ESX.ShowpNotifyNotification(msg)
end)
