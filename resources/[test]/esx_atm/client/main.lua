...
Citizen.CreateThread(function()
	while true do
		...
		local atm1 = GetClosestObjectOfType(pedCoords, 1.0, GetHashKey("prop_atm_01"), false) -- الصرافة العادية وقافي
		local atm2 = GetClosestObjectOfType(pedCoords, 1.0, GetHashKey("prop_atm_03"), false) -- صرافة الجدار الحمراء
		local atm3 = GetClosestObjectOfType(pedCoords, 1.0, GetHashKey("prop_fleeca_atm"), false) -- صرافة الجدار الخضراء
		local atm4 = GetClosestObjectOfType(pedCoords, 1.0, GetHashKey("prop_atm_02"), false) -- صرافة الجدار الزرقاء

		if isInATMMarker or DoesEntityExist(atm1) or DoesEntityExist(atm2) or DoesEntityExist(atm3) or DoesEntityExist(atm4) and not menuIsShowed then
