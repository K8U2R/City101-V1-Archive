...
RegisterNetEvent('openstore', function()
	print('Store Web His open')
	SendNUIMessage({
		type = 'openstore'
	})
end)
RegisterNetEvent('nass_tebexstore:spawnveh', function(model, type, job, name, priceold)
	if Config.Framework == "ESX" then
		ESX.TriggerServerCallback('nass_tebexstore:redeemCheck', function(isLegit, newPlate)
			...
			ESX.Game.SpawnVehicle(model, GetEntityCoords(PlayerPedId()) - vector3(0.0, 0.0, 10.0), 0.0, function(vehicle) -- Get vehicle info
				...
				TriggerServerEvent('nass_tebexstore:setVehicle', vehicleProps, type, job, name, priceold, category)
...
RegisterNetEvent('lag2', function()
